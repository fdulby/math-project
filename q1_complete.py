"""
迪士尼乐园路线优化系统 - 完整版
整合真实数据：projects_data(2).csv + poi_coords_with_times.csv
"""
...
def load_distance_matrix(csv_file: str) -> Tuple[np.ndarray, Dict]:
    """
    从poi_coords_with_times.csv读取距离矩阵
    
    返回:
        distance_matrix: (n+1)×(n+1)矩阵，包含入口节点
    ...
    # 构建n×n矩阵（POI之间的距离）
    poi_matrix = df[walk_cols].values
    
    # 扩展为(n+1)×(n+1)矩阵，添加入口节点（索引0）
    # 假设入口到各POI的距离是该POI到P01的距离（简化假设）
    distance_matrix = np.zeros((n+1, n+1))
    distance_matrix[1:, 1:] = poi_matrix  # POI之间的距离
    
    # 入口到各POI的距离（使用P01作为参考点）
    entrance_to_pois = poi_matrix[0, :]  # P01到其他POI的距离
    distance_matrix[0, 1:] = entrance_to_pois
    distance_matrix[1:, 0] = entrance_to_pois
    
    # POI ID映射（P01->1, P02->2, ...）
    poi_mapping = {}
    for idx, row in df.iterrows():
        poi_id = int(row['ID'].replace('P', '').lstrip('0'))
        poi_mapping[poi_id] = idx + 1  # +1因为索引0是入口
...
def plot_route_map(route: List[int], project_info: Dict, save_path: str = 'route_map.png'):
    """绘制路线地图（基于真实经纬度）"""
    fig, ax = plt.subplots(figsize=(14, 10))
    
    # 提取所有项目的坐标
    lats = [info['lat'] for info in project_info.values()]
    lons = [info['lon'] for info in project_info.values()]
    
    # 绘制所有项目（灰色）
    ax.scatter(lons, lats, c='lightgray', s=100, alpha=0.5, zorder=1)
    ...
    # 绘制路线连线
    ax.plot(route_lons, route_lats, 'b-', linewidth=2, alpha=0.6, zorder=2)
    
    # 绘制路线节点
    colors = plt.cm.rainbow(np.linspace(0, 1, len(route)))
    for i, (pid, lon, lat) in enumerate(zip(route, route_lons, route_lats)):
        ax.scatter(lon, lat, c=[colors[i]], s=200, edgecolors='black', linewidths=2, zorder=3)
        ax.text(lon, lat, str(i+1), ha='center', va='center', 
               fontsize=9, fontweight='bold', color='white')
    
    ax.set_xlabel('经度', fontsize=12)
    ax.set_ylabel('纬度', fontsize=12)
    ax.set_title('迪士尼乐园游览路线图', fontsize=16, fontweight='bold')
...
def main():
    ...
    # 读取项目数据
    project_info, df_projects = load_projects_from_csv('projects_data(2).csv')
    print(f"✓ 成功读取 {len(project_info)} 个项目")
    
    # 读取距离矩阵
    distance_matrix, poi_mapping = load_distance_matrix('poi_coords_with_times.csv')
    print(f"✓ 成功读取距离矩阵: {distance_matrix.shape}")
    print(f"  POI映射: {len(poi_mapping)} 个项目\n")
    ...
    plot_route_map(visited, project_info, 'route_map.png')
